        ...
        if (responseType === 'text') {
            ...
            const needsConnectionHtml = `
                 ...
                 <div class="connect-buttons">
                   ...
                   <button class="skip-btn" onclick="handleSkipConnection(this, '${escapeHtml(agentPrompt)}')">Skip</button>
                 </div>
               `;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Hydrate existing messages (Markdown + Welcome Animation)
    hydrateMessages();
});
...
function hydrateMessages() {
    if (typeof marked === 'undefined') {
        console.error("Marked.js not loaded! Skipping hydration.");
        return;
    }
    
    const bubbles = document.querySelectorAll('.bubble[data-raw]');
    console.log(`Found ${bubbles.length} bubbles to hydrate.`);

    bubbles.forEach(bubble => {
        const rawText = bubble.dataset.raw;
        if (!rawText) {
            console.warn("Bubble has data-raw attribute but it is empty.");
            return;
        }

        // Check if this is the welcome message that needs animation
        if (bubble.dataset.welcomeMessage === "true") {
             // Use typeText for animation
             // Small delay to ensure UI is ready
             setTimeout(() => {
                 typeText(bubble, rawText, 10, () => {
                     // Optional: scroll to bottom after typing
                     const chatBox = document.getElementById('chat-box');
                     if (chatBox) chatBox.scrollTop = chatBox.scrollHeight;
                 });
             }, 100);
        } else {
            // Just parse markdown immediately for history
            try {
...
function handleSkipConnection(btnElement, originalPrompt) {
    // 1. Find the container
    const container = btnElement.closest('.message-content').querySelector('.bubble') || btnElement.closest('.bubble') || btnElement.closest('.connect-buttons').parentNode; 
    
    // 2. Visual feedback: Replace buttons with "Skipped" text
    const buttonGroup = btnElement.closest('.connect-buttons');
    if (buttonGroup) {
        buttonGroup.innerHTML = '<span style="color: #888; font-style: italic;">Connection skipped</span>';
    }

    // 3. Add a system message or agent reply simulation
    const chatBox = document.getElementById('chat-box');
    
    const replyHtml = `
        <div class="message agent-message">
            <div class="avatar">
                <img src="/static/home_page/images/ai_icon.png" alt="AI">
            </div>
            <div class="message-content">
                <div class="bubble">
                    <p>No problem! I can still help you with other tasks, but I won't be able to access or manage your Google Calendar events.</p>
                </div>
            </div>
        </div>
    `;
